...
├── states.md                ← 状态矩阵与告警规则
├── pages/
│   ├── 01-overview.md       ← 总览（进度总览）
│   ├── 02-queues.md         ← 队列列表与详情
│   ├── 03-tasks.md          ← 任务浏览（全局任务视图）+ 任务详情抽屉
│   ├── 05-workers-alerts.md ← Worker 监控 / 告警中心
│   └── 06-guide.md          ← 教程（说人话概念手册）
...
| # | 场景 | 触发 | 频率 | 主页面 |
|---|------|------|------|--------|
| S0 | 盯盘：几十到上百条抓取任务（不同 namespace）同时跑，看每条完成多少/剩多少/预计多久，异常即停 | 调度持续运行期间常开 | 高 | 总览（进度矩阵） |
...
| S5 | 学习：新管理员第一次用，搞懂概念（任务/队列/状态/DLQ）和去哪操作 | 入职/交接时 | 低频 | 教程 |

规模假设 `[假设]`：队列数 30~100+，按 namespace 分组（抓取类型），需要 namespace 级汇总与折叠。
进度口径见 pages/01-overview.md 与 states.md §3.1。

## 3. 页面清单

| 页面 | 路由 | 服务场景 | 频率 | 主行动点 |
|------|------|---------|------|---------|
| 总览（进度总览） | `/` | S0、S1 | 高 | —（状态确认型页面，无强 CTA） |
...
| 教程 | `/guide` | S5 新管理员上手 | 低频 | TOC 锚点导航（纯内容页） |
...
- 左侧固定导航：总览 / 队列 / 任务 / Worker / 告警（告警项带未处理计数红点）/ 教程。
...
<App>
 ...
 └─ <GuidePage>                       # 教程：说人话概念手册（TOC + 状态流转图 + 操作速查表）
 ```

主题：**默认白天（浅色）模式**，顶栏「切换白天/黑夜」按钮随时切换并持久化记忆；两套主题共用同一组语义 token（见 tokens.md），组件不感知主题差异。

## 6. 质量门自查

- [x] S0 盯盘首屏 = 总览进度矩阵（namespace 汇总 + 每队列进度/剩余/速率/ETA/停滞）
...
- 开放问题：进度"本轮期望总量"口径——调度器/期望清单能否提供每轮应完成数（决定进度条是"吞吐相对进度"还是"完成/期望绝对进度"），见 states.md §3.1。
- 开放问题：速率/ETA 当前为前端采样估算 `[假设]`；若要精确吞吐与历史曲线，需后端新增 throughput 聚合接口。
