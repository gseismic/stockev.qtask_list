...
def test_archiver_removes_stale_history_indexes():
    redis_url = os.environ.get("REDIS_URL", "redis://localhost:6379/0")
    r = redis.from_url(redis_url, decode_responses=True)
    q_name = "test_archive_stale_queue"
    idx_key = f"qtask:hist:{q_name}"
    r.delete(idx_key)

    r.zadd(idx_key, {"missing1": time.time() - 86400 * 2, "missing2": time.time() - 86400 * 2})

    archiver = ArchiveManager(redis_url, db_dir="./test_archives_stale")
    count = archiver.archive_to_sqlite(q_name, days_ago=1, batch_size=2)

    assert count == 0
    assert r.zcard(idx_key) == 0

    r.delete(idx_key)
    if os.path.exists("./test_archives_stale"):
        import shutil

        shutil.rmtree("./test_archives_stale")


if __name__ == "__main__":
    test_monitor()
    test_archiver()
    test_archiver_removes_stale_history_indexes()
