...
设计基准：8pt 网格；字阶 1.25 比例；60-30-10 配色分布。深色运维风格，
高密度信息场景下深底降低长时间盯屏疲劳，状态色在深底上使用 500/600 亮度级保证对比度 ≥ 4.5:1。
...
| `--bg-surface-2` | `#1c2129` | 次级面板/表头 |
| `--border-default` | `#30363d` | 描边 |
| `--text-primary` | `#e6edf3` | 正文（对比 13.9:1） |
| `--text-secondary` | `#9198a1` | 次要文字（对比 5.5:1） |
| `--text-faint` | `#6e7681` | 弱提示/时间戳 |

### 语义色（10 + 状态）

| Token | 值 | 语义 |
|---|---|---|
| `--c-primary` | `#2f81f7` | 主操作、链接、选中态 |
| `--c-success` | `#3fb950` | completed、healthy |
| `--c-warning` | `#d29922` | retry_wait/delay、警告告警 |
| `--c-danger` | `#f85149` | dlq/failed、严重告警、危险操作 |
| `--c-muted` | `#8b949e` | skipped/cancelled、禁用 |

各语义色配 `*-dim`（同色 12% 透明度）用于徽章底色，保证文字落在徽章内仍满足对比度。
